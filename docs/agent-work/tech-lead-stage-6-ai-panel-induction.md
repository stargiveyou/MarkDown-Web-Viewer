...
- [ ] **D-1** 미커밋 워킹트리(`claude-cli.ts` export 추출, `format-elapsed.ts`/`.test.ts`, `claude-cli.test.ts` 확장, `BottomAiPanel.tsx` import) 커밋
...
| ADR 신설 | **ADR-011 제안만 한다 (사용자 승인 필요)** | `docs/setting/`은 사용자 승인 없이 수정 금지다. 아래 초안 참조 |
...
### ADR-011 초안 (사용자 승인 전 — `DECISIONS.md`에 아직 쓰지 않았다)
...
- 귀결(제약):
  ...
  2. CLI는 앱의 경로 안전 유틸(`src/lib/path-safety.ts`)을 경유하지 않는 독립 읽기 주체다.
     보안 불변식 2의 봉쇄가 이 경로에는 적용되지 않는다 — 도구 제한·설정 격리로 따로 막아야 한다.
